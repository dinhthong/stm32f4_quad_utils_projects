...
int main(void)
{
    ...
    for (calib_cnt = 0; calib_cnt < 2000; calib_cnt++)
    {
        gyro_signalen();
...
void gyro_signalen() {
    ...
    gyroXrate = MPU6050data[4]/65.5;
    gyroYrate = MPU6050data[5]/65.5;
}
float tmp;
void calculate_angle(void)
{
//calculate angle function
		    ...
		    //2^15-1 divided 500 Full Scale = 65.5
	...
	250Hz=1/4ms | 4ms = sample interval
	65.5=(2^15-1)/500
	angle=angle+omega*t. with omega is the angular velocity.
	*/
		imu_loop=micros();
		elapsed_imu_loop=imu_loop-pre_imu_loop;
		tmp=elapsed_imu_loop*0.000001*500/32767;
	
    angle_roll+= gyro_roll *tmp;// 0.004*500/32767 = 0.000061037
    angle_pitch+= gyro_pitch * tmp;
    angle_yaw+= gyro_yaw * tmp;
		
    //(1/250/65.5)*pi/180=0.000001066 ( radian )
		tmp=(elapsed_imu_loop*0.000001/65.5)*3.141593/180;
    angle_pitch-= angle_roll*sin(gyro_yaw*tmp);//exact complementation when yaw angle changes 
    angle_roll+= angle_pitch*sin(gyro_yaw*tmp);
		pre_imu_loop=imu_loop;
    tong = sqrt((acc_x*acc_x)+(acc_y*acc_y)+(acc_z*acc_z));
    if(abs(acc_x) < tong) {
        angle_pitch_acc=-asin((float)acc_x/tong)*57.296;//calculate pitch angle use accelerometor and Force analysis
    }
    if(abs(acc_y) < tong) {
...
//		  acc_roll -= 0;
    kalAngleX = Kalman_getAngle_roll(angle_roll_acc,gyroXrate,dt);//calculate angle via Kalmal filter with
		// acceleration and gyro as arguments.
    kalAngleY = Kalman_getAngle_pitch(angle_pitch_acc,gyroYrate,dt);

    angle_pitch = angle_pitch * 0.9996 +  kalAngleY * 0.0004;//big coeffient for previous value. and small for newly calculated value.
    angle_roll =  angle_roll * 0.9996 +   kalAngleX * 0.0004 ;
	}
