...
float MS561101BA::getPressure(uint8_t OSR) {
  ...
  int64_t off  = ((uint32_t)_C[1] <<16) + (((int64_t)dT * _CC[3]) >> 7);
  int64_t sens = ((uint32_t)_CC[0] <<15) + (((int64_t)dT * _CC[2]) >> 8);
...
int32_t MS561101BA::getDeltaTemp(uint8_t OSR) {
  ...
  if(rawTemp != NULL) {
    return (int32_t)(rawTemp - ((uint32_t)_CC[4] << 8));
  }
  else {
    return NULL;
  }
