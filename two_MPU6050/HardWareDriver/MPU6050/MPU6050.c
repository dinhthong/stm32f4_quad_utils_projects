...
// Second mpu
uint8_t buffer_2[14], Buf_index_2 = 0;	 //���ݶ�ȡ������
...
{
	IIC2writeBits(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source);
}
...
{
	IIC2writeBits(devAddr, MPU6050_RA_GYRO_CONFIG, MPU6050_GCONFIG_FS_SEL_BIT, MPU6050_GCONFIG_FS_SEL_LENGTH, range);
}
...
{
	IIC2writeBits(devAddr, MPU6050_RA_ACCEL_CONFIG, MPU6050_ACONFIG_AFS_SEL_BIT, MPU6050_ACONFIG_AFS_SEL_LENGTH, range);
}
...
{
	IIC2writeBit(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, enabled);
}
...
{
	IIC2writeBit(devAddr, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_I2C_MST_EN_BIT, enabled);
}
...
{
	IIC2writeBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_I2C_BYPASS_EN_BIT, enabled);
}
...
//ACC_ADC[2]  = az;
	...
	MPU6050_FIFO_2[0][Buf_index_2] = ax;
	MPU6050_FIFO_2[1][Buf_index_2] = ay;
	MPU6050_FIFO_2[2][Buf_index_2] = az;
	MPU6050_FIFO_2[3][Buf_index_2] = gx;
	MPU6050_FIFO_2[4][Buf_index_2] = gy;
	MPU6050_FIFO_2[5][Buf_index_2] = gz;
	// Don't need to reset Buff_index
	Buf_index_2 = (Buf_index_2 + 1) % Buf_Size;//ѭ�����и���
	// ten values with index: 0->9. the 10th index is the Average.
	MPU6050_FIFO_2[0][10] = MPU6050_getAvg(MPU6050_FIFO_2[0], Buf_Size);
	MPU6050_FIFO_2[1][10] = MPU6050_getAvg(MPU6050_FIFO_2[1], Buf_Size);
	MPU6050_FIFO_2[2][10] = MPU6050_getAvg(MPU6050_FIFO_2[2], Buf_Size);
	MPU6050_FIFO_2[3][10] = MPU6050_getAvg(MPU6050_FIFO_2[3], Buf_Size);
	MPU6050_FIFO_2[4][10] = MPU6050_getAvg(MPU6050_FIFO_2[4], Buf_Size);
	MPU6050_FIFO_2[5][10] = MPU6050_getAvg(MPU6050_FIFO_2[5], Buf_Size);
}
...
void MPU6050_initialize_2(void)
{
	int16_t temp[6];
	unsigned char i;
	MPU6050_setClockSource_2(MPU6050_CLOCK_PLL_XGYRO); 
	MPU6050_setFullScaleGyroRange_2(MPU6050_GYRO_FS_2000);
	MPU6050_setFullScaleAccelRange_2(MPU6050_ACCEL_FS_2);	//���ٶȶ�������� +-2G
	...
	//����MPU6050 ���ж�ģʽ ���жϵ�ƽģʽ
	IIC2writeBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_LEVEL_BIT, 0);
	IIC2writeBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_OPEN_BIT, 0);
	IIC2writeBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_LATCH_INT_EN_BIT, 1);
	IIC2writeBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_RD_CLEAR_BIT, 1);
	// When DATA is READY -> DRY pin <-> PC9 is set
	IIC2writeBit(devAddr, MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DATA_RDY_BIT, 1);
	//����MPU6050�ĵ�ͨ�˲����������˻����𶯶���̬�����Ӱ��
	IIC2writeBits(devAddr, MPU6050_RA_CONFIG, 7, 8, MPU6050_DLPF); //����MPU6050��ͨ�˲�
	
	// get inital 10 values into the buffer
	for(i = 0; i < 10; i++) //����FIFO����
	{
		delay_us(50);
		MPU6050_getMotion6_2(&temp[0], &temp[1], &temp[2], &temp[3], &temp[4], &temp[5]);
	}
}
...
int16_t MPU6050_Lastax_2, MPU6050_Lastay_2, MPU6050_Lastaz_2  //�ϴ�ֵ
...
//	if(MPU6050_is_DRY())
//	{
		IIC2readBytes(devAddr, MPU6050_RA_ACCEL_XOUT_H, 14, buffer_2);
		MPU6050_Lastax_2 = (((int16_t)buffer_2[0]) << 8) | buffer_2[1];
		MPU6050_Lastay_2 = (((int16_t)buffer_2[2]) << 8) | buffer_2[3];
		MPU6050_Lastaz_2 = (((int16_t)buffer_2[4]) << 8) | buffer_2[5];
		//�����¶�ADC
		MPU6050_Lastgx_2 = (((int16_t)buffer_2[8]) << 8) | buffer_2[9];
		MPU6050_Lastgy_2 = (((int16_t)buffer_2[10]) << 8) | buffer_2[11];
		MPU6050_Lastgz_2 = (((int16_t)buffer_2[12]) << 8) | buffer_2[13];
		MPU6050_newValues_2(MPU6050_Lastax_2, MPU6050_Lastay_2, MPU6050_Lastaz_2
				  , MPU6050_Lastgx_2, MPU6050_Lastgy_2, MPU6050_Lastgz_2);
		// Average calculated raw values
		*ax  = MPU6050_FIFO_2[0][10];
		*ay  = MPU6050_FIFO_2[1][10];
		*az = MPU6050_FIFO_2[2][10];
		*gx  = MPU6050_FIFO_2[3][10];
		*gy = MPU6050_FIFO_2[4][10];
		*gz = MPU6050_FIFO_2[5][10];
	//else get the old values
//	}
//	else
//	{
